...
```{r}
cluster_iter = function(iter = 1, n_obs = 10000, n_cluster = 20) {
...
I introduced treatment that varies across $i$ and $c$.

## Visualization

#### Simulate n times

```{r, warning=F, results=F, cache = T}
sim_list <- map(1:1000, cluster_iter)
...
if (input$points) {
  g0 = g0 +
    geom_point(data = subset(sim_df, se_type %in% c("normal")), aes(x = rho, y = diff), shape = 4, size = 0.5) +
    geom_point(data = subset(sim_df, se_type %in% c("normal")), aes(x = rho, y = upper), color = grey_mid, size = 0.5) +
    geom_point(data = subset(sim_df, se_type %in% c("normal")), aes(x = rho, y = lower), color = grey_mid, size = 0.5) +
    geom_point(data = subset(sim_df, se_type %in% c("clustered")), aes(x = rho, y = diff), size = 0.5) +
    geom_point(data = subset(sim_df, se_type %in% c("clustered")), aes(x = rho, y = upper), color = grey_dark, size = 0.5) +
    geom_point(data = subset(sim_df, se_type %in% c("clustered")), aes(x = rho, y = lower), color = grey_dark, size = 0.5) +
    geom_smooth(data = subset(sim_df, se_type %in% c("normal")),
                aes(x = rho, y = upper), se = FALSE, color = red) +
    geom_smooth(data = subset(sim_df, se_type %in% c("normal")),
                aes(x = rho, y = lower), se = FALSE, color = red) +
    geom_smooth(data = subset(sim_df, se_type %in% c("clustered")),
