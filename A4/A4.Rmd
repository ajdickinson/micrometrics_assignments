...
library(pacman)
p_load(fixest, mvtnorm, purrr, furrr, viridis, ggplot2, tidyverse, broom)
# Define pink color
red_pink <- "#e64173"
turquoise <- "#20B2AA"
orange <- "#FFA500"
...
g0 = ggplot(data = sim_df %>% filter(se_type == "normal")) +
  ...
  geom_point(aes(x = rho, y = upper), color = blue) +
  geom_point(aes(x = rho, y = lower), color = blue) +
